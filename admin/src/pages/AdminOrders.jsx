    ...
    const fetchOrders = useCallback(async () => {
        try {
            ...
            if (err.response?.status === 403) {
                setError("Access denied. Admin / Owner permission required.");
            } else {
            ...
            <div className="admin-root max-w-6xl mx-auto px-4 py-8">
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
                    {[
                        ...
                        { label: "Revenue", value: `₹${stats.revenue.toLocaleString("en-IN")}`, color: "text-blue-600" },
                            ...
                            <div key={order._id} className="bg-white rounded-2xl border border-stone-200 shadow-sm overflow-hidden">
                                ...
                                <div className="px-5 pb-3">
                                    <div className="flex items-center gap-0">
                                        {FLOW_STEPS.map((step, i) => (
                                            <div key={step} className="flex items-center flex-1">
                                                <div className={`w-4 h-4 rounded-full border-2 flex items-center justify-center shrink-0 transition-all ${i <= stepIdx
                                    ...
                                    <div className="flex justify-between mt-1">
                                        {FLOW_STEPS.map((step, i) => (
                                            <p key={step} className={`text-[9px] font-medium ${i <= stepIdx ? "text-emerald-600" : "text-zinc-300"}`} style={{ width: `${100 / FLOW_STEPS.length}%`, textAlign: i === 0 ? "left" : i === FLOW_STEPS.length - 1 ? "right" : "center" }}>
                                            ...
                                            <div className="bg-stone-50 rounded-xl p-4">
                                                ...
                                                {nextStatus ? (
                                                    <button
                                                        ...
                                                        disabled={isUpdating}
                                                        className={`w-full flex items-center justify-center gap-2 py-3 rounded-xl text-sm font-bold transition-all active:scale-95 disabled:opacity-60 ${nextCfg ? "bg-zinc-900 text-white hover:bg-zinc-800" : ""}`}
                                                    >
                                                        {isUpdating ? (
